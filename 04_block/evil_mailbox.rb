...
class EvilMailbox
  def initialize(client, secret = nil)
    @client = client
    client.auth(secret) unless secret.nil?
    @handler = ->(body) { secret.nil? ? body : body + secret }
  end

  def send_mail(subject, body)
    result = @client.send_mail(subject, @handler.call(body))
